...
static void terminal_cmd_brake_status(int argc, const char **argv) {
    ...
    if (argc > 1) {     // parse command argument
        ...
        commands_printf("   HW id: %08x %08x %08x", __REV(STM32_UUID[0]), __REV(STM32_UUID[1]), __REV(STM32_UUID[2]) );
        commands_printf("   CURRENT_SHUNT_RES: %.1f uOhm", (double)(CURRENT_SHUNT_RES*1e6));
